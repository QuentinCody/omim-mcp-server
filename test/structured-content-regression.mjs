...
/**
 * Regression tests for omim-mcp-server structuredContent responses.
 */
...
if (index) {
  assertContains('src/index.ts', index, 'OmimDataDO', 'index exports OmimDataDO');
...
if (codeMode) {
  assertContains('src/tools/code-mode.ts', codeMode, 'omim_search', 'code-mode registers omim_search');
  assertContains('src/tools/code-mode.ts', codeMode, 'omim_execute', 'code-mode registers omim_execute');
}

if (queryData) {
  assertContains('src/tools/query-data.ts', queryData, 'omim_query_data', 'registers omim_query_data');
}

if (getSchema) {
  assertContains('src/tools/get-schema.ts', getSchema, 'omim_get_schema', 'registers omim_get_schema');
}
