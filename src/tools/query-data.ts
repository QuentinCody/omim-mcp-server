...
interface QueryEnv {
    OMIM_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv) {
    const handler = createQueryDataHandler("OMIM_DATA_DO", "omim");

    server.registerTool(
        "omim_query_data",
        {
            title: "Query Staged OMIM Data",
            description:
                "Query staged OMIM data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
